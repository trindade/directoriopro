...
class UserController extends Controller
{
    ...
    */
    ...
    public function commentAction($user_id, $comment_id)
    {
	    ...
	    // existe comentario?
	    $query = $em->createQuery("SELECT u.name, u.twitter_url, u.category_id, u.slug, c.id, c.from_id, c.body, c.type, c.date FROM ApplicationUserBundle:User u, ApplicationUserBundle:Comment c WHERE u.id = c.from_id AND c.to_id = :to_id AND c.id = :id ORDER BY c.id DESC");
	    $query->setParameters(array(
	      'id' => $comment_id,
	      'to_id' => $user_id
	    ));
